...
## WHY
AI **Co-Founder** web app (PWA) for Indonesian solopreneurs / UMKM with no business background — takes them from raw idea → validated research → business plan + financial model → brand identity → investor-grade proposal & pitch deck. Part of the **AgentBuff** suite (POS, Absent, Agentic). **Free, multi-provider BYOK** (Gemini API key / OpenAI API key / Codex Sign-in). **MCP-native** (the engine is a callable backend for external agents, not just a UI). See §1, §1.5.
...
## CURRENT STATE (Fase 0 → Fase 1)
**Full map: [docs/IMPLEMENTATION-STATUS.md](docs/IMPLEMENTATION-STATUS.md) · run/test: [docs/RUNNING.md](docs/RUNNING.md).** Status: **130 tests passing · 100% coverage on both deterministic engines · `tsc` strict clean · `next build` green (14 routes).** Runs **end-to-end** (link a BYOK key — Gemini / OpenAI / **Codex** — → create project → real grounded validate + plan). Persistence auto-switches: **Postgres (Prisma) when `DATABASE_URL` is set** (verified against the VPS DB), else in-memory. Auth = signed-cookie guest session (Google OIDC pending creds — see [docs/AUTH-SETUP.md](docs/AUTH-SETUP.md)).

**Done & tested:** Financial + ValidationScore engines (100%, adversarially hardened) · BYOK envelope encryption ([src/lib/crypto/](src/lib/crypto/)) · GeminiAdapter + OpenAIAdapter (REST, contract tests) · Provider Registry ([src/lib/ai/registry.ts](src/lib/ai/registry.ts)) · domain + 3 services ([src/server/services/](src/server/services/)) · **full MCP tool catalog** ([src/server/mcp/](src/server/mcp/)) · **Prisma schema** ([prisma/schema.prisma](prisma/schema.prisma)) · **Next.js PWA** ([src/app/](src/app/)): landing, onboarding (real BYOK validation via `/api/byok/validate`), dashboard, and a **wired financial calculator** (`/api/financials` → engine).

**Setup notes:** pnpm uses `node-linker=hoisted` ([.npmrc](.npmrc)) so Turbopack resolves on Windows; `next.config.ts` pins `turbopack.root`. Tailwind v4 (CSS-first, tokens in [src/app/globals.css](src/app/globals.css)).

**Next (tracked):** domain model + repositories (§11) → service layer (Research/Planner/Brand/Docs orchestration) → full MCP tool catalog + Streamable-HTTP/OAuth → provider Deep-Research/image/doc methods (currently 501 stubs) → PDF render worker → Next.js app + PWA + Prisma + auth. The I/O boundaries (DB, OAuth, storage, Chromium, live LLM) sit behind interfaces with in-memory/test impls; they go live when credentials/infra are provided. Scaffold larger pieces with plan mode + approval first.
