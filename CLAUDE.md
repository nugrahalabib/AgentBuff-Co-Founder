...
**Full map: [docs/IMPLEMENTATION-STATUS.md](docs/IMPLEMENTATION-STATUS.md) · run/test: [docs/RUNNING.md](docs/RUNNING.md).** Status: **130 tests passing · 100% coverage on both deterministic engines · `tsc` strict clean · `next build` green (14 routes).** Runs **end-to-end with zero infra** (no-DB runtime + signed-cookie session; link a BYOK key → create project → real grounded validate + plan).
