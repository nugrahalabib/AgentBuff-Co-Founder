...
export class McpGatewayService {
  ...
  async issueToken(userId: string, name: string): Promise<{ id: string; token: string; display: string }> {
    ...
    return { id, token: issued.token, display: issued.display };
  }
  ...
  async authenticate(token: string): Promise<AuthenticatedClient | null> {
    ...
    return { userId: record.ownerUserId, clientId: record.id };
  }
